...
fn test_parse_entries () {
    ...
    assert_eq!(entries[0].borrow().title.as_str(), "foo");
    assert_eq!(entries[0].borrow().url.as_str(), "foo");
    assert_eq!(entries[0].borrow().username.as_str(), "foo");
    assert_eq!(entries[0].borrow().password.string.as_str(), "DLE\"H<JZ|E");
...
fn test_create_group_tree() {
    ...
    assert_eq!(get_parent_title(1, &db).as_str(), "Internet");
    assert_eq!(get_parent_title(2, &db).as_str(), "Internet");
    assert_eq!(get_children_title(2, 0, &db).as_str(), "22");
    assert_eq!(get_children_title(2, 1, &db).as_str(), "21");
    assert_eq!(get_parent_title(3, &db).as_str(), "11");
    assert_eq!(get_parent_title(4, &db).as_str(), "11");
    assert_eq!(get_children_title(4, 0, &db).as_str(), "32");
    assert_eq!(get_children_title(4, 1, &db).as_str(), "31");
    assert_eq!(get_parent_title(5, &db).as_str(), "21");
    assert_eq!(get_parent_title(6, &db).as_str(), "21");
